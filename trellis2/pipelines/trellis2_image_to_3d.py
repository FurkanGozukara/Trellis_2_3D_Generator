    ...
    def run(
        ...
        if self.low_vram and coords.shape[0] > 24000 and pipeline_type in ['1024', '1024_cascade', '1536_cascade', '2048_cascade']:
            if 'shape_slat_flow_model_512' in self.models:
                print(
                    f"Token count {coords.shape[0]} > 24000. "
                    "Switching to 512g_1024t pipeline to save memory."
                )
                pipeline_type = '512g_1024t'
                coords = (coords // 2).unique(dim=0)
            else:
                print(
                    f"Token count {coords.shape[0]} > 24000, but 'shape_slat_flow_model_512' "
                    "is not loaded. Continuing with the original pipeline."
                )
        if pipeline_type == '512':
